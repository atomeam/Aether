...
$checks = @(
    ...
    @{ Name = "Environment Validation"; Script = "$scriptDir\..\env-validation\skill.ps1"; Args = @("-Audit"); Critical = $true },
...
& "$scriptDir\..\real-vs-simulated\skill.ps1"
if (`$LASTEXITCODE -ne 0) {
    Write-Host "❌ Pre-commit checks failed. Commit blocked." -ForegroundColor Red
    exit 1
}

& "$scriptDir\..\env-validation\skill.ps1" -Audit
if (`$LASTEXITCODE -ne 0) {
    Write-Host "❌ Pre-commit checks failed. Commit blocked." -ForegroundColor Red
    exit 1
}
