    ...
    case 'tools/call':
      ...
      if (name === 'execute_powershell_bus') {
        return new Promise((resolve) => {
          ...
          exec(args.command, (error: Error | null, stdout: string, stderr: string) => {
            if (error) {
...
async function startServer() {
  ...
  app.post("/api/git/commit", async (req, res) => {
    ...
    try {
      ...
      exec(commands.join(" && "), (error: Error | null, stdout: string, stderr: string) => {
        if (error) {
  ...
  async function getGitBranch(): Promise<string> {
    return new Promise((resolve) => {
      exec("git rev-parse --abbrev-ref HEAD", (error: Error | null, stdout: string) => {
  ...
  async function getGitStatus(): Promise<string> {
    return new Promise((resolve) => {
      exec("git status --short", (error: Error | null, stdout: string) => {
...
app.get("/api/dream", async (req, res) => {
  try {
    ...
    const statusWithShouldDream = { ...status, shouldDream: shouldDream() };
