...
var fs2, import_meta, LOG_DIR, DEFAULT_CAPS, CYCLE_STATE_FILE;
...
var fs3, import_meta2, LOG_DIR2, QUARANTINE_FILE;
...
var fs4, import_meta3, LOG_DIR3, CANARY_LOG, DOCS_DIR, DEFAULT_CANARY_CONFIG;
...
var fs5, import_meta4, LOG_DIR4, CHECKPOINT_DIR, REVERT_LOG, DEFAULT_REVERT_THRESHOLDS, STATE_FILE;
...
function executeChaos(scenario, targetPath) {
  ...
  const fullPath = import_path2.default.resolve(process.cwd(), sandboxPath);
  ...
  switch (scenario) {
    case "broken_package_json": {
      ...
      import_fs2.default.writeFileSync(import_path2.default.join(fullPath, "package.json"), pkg);
      return {
        status: "success",
    ...
    case "corrupted_env_var": {
      import_fs2.default.writeFileSync(import_path2.default.join(fullPath, ".env"), "PORT=not_a_number\nDEBUG=invalid\n");
      return {
        status: "success",
    ...
    case "invalid_syntax": {
      import_fs2.default.writeFileSync(import_path2.default.join(fullPath, "broken.js"), "const x == 5;\nexport default x;");
      return {
        status: "success",
    ...
    case "network_timeout": {
      import_fs2.default.writeFileSync(import_path2.default.join(fullPath, "timeout.sh"), '#!/bin/bash\necho "Simulated timeout"\nsleep 300\n');
      return {
        status: "success",
...
function cleanupSandbox(targetPath) {
  ...
  const fullPath = import_path2.default.resolve(process.cwd(), sandboxPath);
  if (import_fs2.default.existsSync(fullPath)) {
    import_fs2.default.rmSync(fullPath, { recursive: true, force: true });
  }
  return { cleaned: sandboxPath };
}
var import_fs2, import_path2, SANDBOX_PATHS;
var init_src2 = __esm({
  "../../packages/chaos/src/index.ts"() {
    "use strict";
...
function getSandboxRoot(profileId) {
  ...
  return import_path3.default.join(config.basePath, profileId);
}
function resolveSandboxPath(profileId, requestedPath) {
  const sandboxRoot = getSandboxRoot(profileId);
  const absPath = import_path3.default.isAbsolute(requestedPath) ? requestedPath : import_path3.default.resolve(process.cwd(), requestedPath);
  const normalized = import_path3.default.normalize(absPath);
  const normalizedRoot = import_path3.default.normalize(sandboxRoot);
...
function createSandbox(profileId) {
  ...
  try {
    if (!import_fs3.default.existsSync(root)) {
      import_fs3.default.mkdirSync(root, { recursive: true });
      import_fs3.default.mkdirSync(import_path3.default.join(root, "workspace"), { recursive: true });
      import_fs3.default.mkdirSync(import_path3.default.join(root, "temp"), { recursive: true });
    }
...
function deleteSandbox(profileId) {
  ...
  try {
    if (import_fs3.default.existsSync(root)) {
      import_fs3.default.rmSync(root, { recursive: true, force: true });
    }
    return { success: true };
  } catch (e) {
    return { success: false, error: e.message };
...
function listSandboxes() {
  if (!import_fs3.default.existsSync(config.basePath)) return [];
  return import_fs3.default.readdirSync(config.basePath).filter((stat) => {
    try {
      return import_fs3.default.statSync(import_path3.default.join(config.basePath, stat)).isDirectory();
    } catch {
      return false;
    }
  });
...
function logEscapeAttempt(options) {
  ...
  const dir = import_path3.default.dirname(ESCAPE_LOG_PATH);
  if (!import_fs3.default.existsSync(dir)) import_fs3.default.mkdirSync(dir, { recursive: true });
  import_fs3.default.appendFileSync(ESCAPE_LOG_PATH, JSON.stringify(entry) + "\n");
}
...
var import_fs3, import_path3, import_crypto, DEFAULT_CONFIG, config, DEFAULT_PATH_POLICY, ESCAPE_LOG_PATH;
var init_src3 = __esm({
  "../../packages/sandbox/src/index.ts"() {
    "use strict";
    ...
    ESCAPE_LOG_PATH = import_path3.default.resolve(process.cwd(), "../../logs/sandbox-escapes.jsonl");
  }
});
...
function ensureDir() {
  const dir = import_path4.default.dirname(AUDIT_PATH);
  if (!import_fs4.default.existsSync(dir)) import_fs4.default.mkdirSync(dir, { recursive: true });
}
function getPolicyHash() {
  try {
    const policyContent = import_fs4.default.readFileSync("../../packages/curator/policy.yaml", "utf-8");
...
function loadChainState() {
  if (!import_fs4.default.existsSync(CHAIN_STATE_PATH)) return { lastHash: "", lastIndex: 0, lastPolicyHash: "" };
  return JSON.parse(import_fs4.default.readFileSync(CHAIN_STATE_PATH, "utf-8"));
}
function saveChainState(state) {
  const dir = import_path4.default.dirname(CHAIN_STATE_PATH);
  if (!import_fs4.default.existsSync(dir)) import_fs4.default.mkdirSync(dir, { recursive: true });
  import_fs4.default.writeFileSync(CHAIN_STATE_PATH, JSON.stringify(state));
}
...
function logDecision(input) {
  ...
  import_fs4.default.appendFileSync(AUDIT_PATH, JSON.stringify(record2) + "\n");
...
function verifyChainIntegrity() {
  if (!import_fs4.default.existsSync(AUDIT_PATH)) return { valid: true, errors: [] };
  const chainState = loadChainState();
  const content = import_fs4.default.readFileSync(AUDIT_PATH, "utf-8");
...
function getDecisions(options) {
  ...
  if (!import_fs4.default.existsSync(AUDIT_PATH)) return Promise.resolve([]);
  const content = import_fs4.default.readFileSync(AUDIT_PATH, "utf-8");
...
var import_zod4, import_fs4, import_path4, import_crypto2, AuditRecordSchema, AUDIT_PATH, CHAIN_STATE_PATH;
var init_src4 = __esm({
  "../../packages/curator-audit/src/index.ts"() {
    "use strict";
    ...
    AuditRecordSchema = import_zod4.z.object({
      decisionId: import_zod4.z.string(),
      runId: import_zod4.z.string().optional(),
      timestamp: import_zod4.z.string(),
      actor: import_zod4.z.enum(["executor", "evaluator", "reflector", "human", "system"]),
      tool: import_zod4.z.string(),
      args: import_zod4.z.record(import_zod4.z.unknown()).optional(),
      decision: import_zod4.z.enum(["approve", "deny", "escalate"]),
      rule: import_zod4.z.string().optional(),
      confidence: import_zod4.z.number().min(0).max(1).optional(),
      reason: import_zod4.z.string().optional(),
      hash: import_zod4.z.string().optional(),
      previousHash: import_zod4.z.string().optional(),
      policyHash: import_zod4.z.string().optional()
    });
    AUDIT_PATH = import_path4.default.resolve(process.cwd(), "../../logs/curator-audit.jsonl");
    CHAIN_STATE_PATH = import_path4.default.resolve(process.cwd(), "../../logs/audit-chain-state.json");
  }
});
...
function ensureDir2() {
  const dir = import_path5.default.dirname(LESSONS_PATH);
...
function writeLesson(input) {
  ...
  import_fs5.default.appendFileSync(LESSONS_PATH, line);
  return lesson;
}
function readLessons(options) {
  const { since, pattern, limit = 100 } = options || {};
  if (!import_fs5.default.existsSync(LESSONS_PATH)) {
    return Promise.resolve([]);
  }
  const content = import_fs5.default.readFileSync(LESSONS_PATH, "utf-8");
  const lines = content.trim().split("\n").filter(Boolean);
...
var import_zod5, import_fs5, import_path5, import_crypto3, LessonSchema, WriteLessonInput, LESSONS_PATH;
var init_src6 = __esm({
  "../../packages/lessons/src/index.ts"() {
    "use strict";
    ...
    LessonSchema = import_zod5.z.object({
      id: import_zod5.z.string(),
      pattern: import_zod5.z.string(),
      // e.g., "npm error", "E404", "422"
      suggestion: import_zod5.z.string(),
      // what Evaluator suggested
      action: import_zod5.z.string(),
      // what Executor did
      outcome: import_zod5.z.enum(["success", "failure", "noop"]),
    ...
    LESSONS_PATH = import_path5.default.resolve(process.cwd(), "../../logs/lessons.jsonl");
  }
});
...
var init_src7 = __esm({
  "../../packages/mcp-tools/src/index.ts"() {
    ...
    getAgentStateTool = {
      ...
      async execute(args) {
        const metrics2 = await Promise.resolve().then(() => (init_src5(), src_exports4));
...
var import_zod6, StepSchema, WorkflowSchema, EXAMPLE_WORKFLOWS;
var init_src8 = __esm({
  "../../packages/workflow/src/index.ts"() {
    "use strict";
    import_zod6 = require("zod");
    init_src7();
    StepSchema = import_zod6.z.object({
      name: import_zod6.z.string(),
      tool: import_zod6.z.string(),
      args: import_zod6.z.record(import_zod6.z.unknown()),
      condition: import_zod6.z.string().optional(),
      // JMESPath expression
      retry: import_zod6.z.number().optional()
    });
    WorkflowSchema = import_zod6.z.object({
      name: import_zod6.z.string(),
      trigger: import_zod6.z.object({
        type: import_zod6.z.enum(["webhook", "schedule", "event"]),
        url: import_zod6.z.string().optional(),
        // For webhook
        schedule: import_zod6.z.string().optional(),
        // Cron
        event: import_zod6.z.string().optional()
        // Event type
      }),
      steps: import_zod6.z.array(StepSchema),
      on_failure: import_zod6.z.enum(["stop", "continue", "rollback"]).default("stop"),
      timeout: import_zod6.z.number().default(3e5)
      // 5 min
    });
    EXAMPLE_WORKFLOWS = [
      {
...
var init_src15 = __esm({
  "../../packages/alerts/src/index.ts"() {
    ...
    AlertEngine = class extends import_events3.EventEmitter {
      ...
      async evaluate() {
        const results = [];
        const { getStats: getStats4 } = await Promise.resolve().then(() => (init_src4(), src_exports3));
...
async function collectTelemetry() {
  ...
  try {
    const { snapshot: snapshot2 } = await Promise.resolve().then(() => (init_src5(), src_exports4));
  ...
  try {
    const { getLearnedPatterns: getLearnedPatterns3 } = await Promise.resolve().then(() => (init_src6(), src_exports5));
...
async function generateAutoJournal() {
  ...
  try {
    const { readLessons: readLessons2 } = await Promise.resolve().then(() => (init_src6(), src_exports5));
...
var import_zod7, import_fs16, import_path16, import_crypto21, AssistantIdentitySchema, ParticipantVoteSchema, ConveneSessionSchema, SCOPES, CONVENE_PATH, ASSISTANTS_PATH;
var init_src33 = __esm({
  "../../packages/convene/src/index.ts"() {
    ...
    AssistantIdentitySchema = import_zod7.z.object({
      id: import_zod7.z.string(),
      name: import_zod7.z.string(),
      scopes: import_zod7.z.array(import_zod7.z.string()),
      // payments, support, code, etc.
      signedClaim: import_zod7.z.string().optional()
      // Signed assertion of identity
    });
    ParticipantVoteSchema = import_zod7.z.object({
      assistantId: import_zod7.z.string(),
      assistantName: import_zod7.z.string(),
      scope: import_zod7.z.string(),
      vote: import_zod7.z.enum(["approve", "deny", "abstain", "escalate"]),
      confidence: import_zod7.z.number().min(0).max(1),
      rationale: import_zod7.z.string()
    });
    ConveneSessionSchema = import_zod7.z.object({
      sessionId: import_zod7.z.string(),
      profileId: import_zod7.z.string(),
      question: import_zod7.z.string(),
      context: import_zod7.z.record(import_zod7.z.unknown()),
      requiredScopes: import_zod7.z.array(import_zod7.z.string()),
      votes: import_zod7.z.array(ParticipantVoteSchema),
      consensus: import_zod7.z.number().min(0).max(1),
      recommended: import_zod7.z.enum(["approve", "deny", "escalate"]),
      resolution: import_zod7.z.enum(["auto_executed", "escalated_to_triage", "rejected", "pending"]),
      narrative: import_zod7.z.string(),
      createdAt: number,
      resolvedAt: number
    });
    SCOPES = {
...
var ComponentSchema = import_zod2.z.object({
  id: import_zod2.z.string().min(1),
  type: import_zod2.z.enum(["stat", "chart", "list", "status", "gauge"]),
  title: import_zod2.z.string().min(1),
  props: import_zod2.z.object({
    label: import_zod2.z.string(),
    value: import_zod2.z.string().optional(),
    items: import_zod2.z.array(import_zod2.z.string()).optional(),
...
var BuildRequestSchema = import_zod2.z.object({
  prompt: import_zod2.z.string().min(1).max(5e3),
  currentComponents: import_zod2.z.array(ComponentSchema).default([])
});
var ComponentActionSchema = import_zod2.z.union([
  import_zod2.z.object({
    action: import_zod2.z.literal("ADD"),
    plan: ComponentSchema
  }),
  import_zod2.z.object({
    action: import_zod2.z.literal("REMOVE"),
    targetId: import_zod2.z.string()
  }),
  import_zod2.z.object({
    action: import_zod2.z.literal("MODIFY"),
    targetId: import_zod2.z.string(),
    plan: ComponentSchema.partial()
  }),
  // Extended UI actions
  import_zod2.z.object({
    action: import_zod2.z.literal("PATCH"),
    targetId: import_zod2.z.string(),
    patchData: import_zod2.z.record(import_zod2.z.unknown())
  }),
  import_zod2.z.object({
    action: import_zod2.z.literal("MUTATE_THEME"),
    themeUpdate: import_zod2.z.record(import_zod2.z.unknown())
  }),
  import_zod2.z.object({
    action: import_zod2.z.literal("SET_DIRECTIVE"),
    directive: import_zod2.z.string()
  }),
  import_zod2.z.object({
    action: import_zod2.z.literal("SOURCE_MUTATION")
  }),
  // MCP tool invocations
  import_zod2.z.object({
    action: import_zod2.z.literal("MCP_TOOL_CALL"),
    toolName: import_zod2.z.string(),
    toolArgs: import_zod2.z.record(import_zod2.z.unknown())
  })
]);
var CouncilSchema = import_zod2.z.object({
  builder: import_zod2.z.string(),
  strategist: import_zod2.z.string(),
  operator: import_zod2.z.string()
});
var BuildResponseSchema = import_zod2.z.object({
  thought: import_zod2.z.string(),
  explanation: import_zod2.z.string(),
  actions: import_zod2.z.array(ComponentActionSchema),
  isFallback: import_zod2.z.boolean().default(false),
  quotaExhausted: import_zod2.z.boolean().optional(),
  curatorRejected: import_zod2.z.boolean().optional(),
  council: CouncilSchema.optional(),
  manifesto: import_zod2.z.string().optional()
});
...
var CapabilityTag = import_zod3.z.enum([
  "ui:render",
  "ui:interactive",
  "read:state",
  "write:state",
...
var PropSpecSchema = import_zod3.z.object({
  type: import_zod3.z.enum(["string", "number", "boolean", "enum", "array"]),
  required: import_zod3.z.boolean().default(false),
  enum: import_zod3.z.array(import_zod3.z.string()).optional(),
  description: import_zod3.z.string().optional()
});
var ComponentManifestEntry = import_zod3.z.object({
  type: import_zod3.z.string(),
  description: import_zod3.z.string(),
  propsSchema: import_zod3.z.record(import_zod3.z.string(), PropSpecSchema),
  capabilities: import_zod3.z.array(CapabilityTag).default(["ui:render"]),
  maxPerResponse: import_zod3.z.number().int().positive().optional()
});
var ComponentManifest = import_zod3.z.array(ComponentManifestEntry);
var MANIFEST = [
  {
    type: "stat",
  ...
  switch (method) {
    ...
    case "tools/call":
      ...
      if (name === "read_workspace_file") {
        ...
        return { content: import_node_fs.default.readFileSync(fullPath, "utf8") };
      }
      if (name === "write_workspace_file") {
        const fullPath = import_node_path.default.join(process.cwd(), args.path);
        if (!fullPath.startsWith(process.cwd())) throw new Error("Security Violation: Out of bounds write.");
        import_node_fs.default.writeFileSync(fullPath, args.content);
      ...
      if (name === "execute_powershell_bus") {
        return new Promise((resolve2) => {
          ...
          (0, import_node_child_process.exec)(args.command, (error, stdout, stderr) => {
            if (error) {
...
`);
  ...
  app.get("/api/agents/curator/decisions", async (req, res) => {
    try {
      const { getDecisions: getDecisions2, getStats: getStats4 } = await Promise.resolve().then(() => (init_src4(), src_exports3));
  ...
  app.post("/api/build", async (req, res) => {
    ...
    try {
      ...
      Promise.resolve(commitToLedger({
        traceId,
        prompt,
        promptHash,
        verdict: verdict.approved ? "APPROVED" : "REJECTED",
  ...
  app.post("/api/evolve", async (req, res) => {
    try {
      ...
      const cpuLoad = import_node_os.default.loadavg()[0];
      const freeMemMB = Math.round(import_node_os.default.freemem() / 1024 / 1024);
      const homeBaseStatus = cpuLoad < 2 ? "SYNCED" : "LOAD_WARNING";
      const envContext = `[ENVIRONMENT]: Node: ${import_node_os.default.hostname()}, OS: ${import_node_os.default.type()}, CPU_Load: ${cpuLoad.toFixed(2)}, FreeMem: ${freeMemMB}MB.`;
      ...
      let srcDNA = "";
      try {
        srcDNA = import_node_fs.default.readFileSync(import_node_path.default.join(process.cwd(), "src/App.tsx"), "utf-8");
      } catch (e) {
  ...
  app.post("/api/git/commit", async (req, res) => {
    ...
    try {
      for (const file of files) {
        ...
        import_node_fs.default.writeFileSync(fullPath, file.content);
      }
      const commands = [
      ...
      (0, import_node_child_process.exec)(commands.join(" && "), (error, stdout, stderr) => {
        if (error) {
  ...
  async function getGitBranch() {
    return new Promise((resolve2) => {
      (0, import_node_child_process.exec)("git rev-parse --abbrev-ref HEAD", (error, stdout) => {
  ...
  async function getGitStatus() {
    return new Promise((resolve2) => {
      (0, import_node_child_process.exec)("git status --short", (error, stdout) => {
...
app.get("/api/dream", async (req, res) => {
  try {
    ...
    const statusWithShouldDream = { ...status, shouldDream: shouldDream2() };
    if (req.query.trigger === "true" && shouldDream2()) {
      const result = await dream2();
      return res.json({ ...statusWithShouldDream, triggered: result });
    }
    res.json(statusWithShouldDream);
  } catch (e) {
    res.json({ error: e.message });
  }
});
...
app.get("/api/audit-verify", async (req, res) => {
  try {
    const { verifyChainIntegrity: verifyChainIntegrity2 } = await Promise.resolve().then(() => (init_src4(), src_exports3));
...
app.get("/api/sandbox/config", async (req, res) => {
  try {
    const { getConfig: getConfig2, getPathPolicy: getPathPolicy2 } = await Promise.resolve().then(() => (init_src3(), src_exports2));
...
app.post("/api/sandbox/config", async (req, res) => {
  try {
    const { setConfig: setConfig2 } = await Promise.resolve().then(() => (init_src3(), src_exports2));
...
app.get("/api/sandbox/:profileId", async (req, res) => {
  try {
    const { createSandbox: createSandbox2, deleteSandbox: deleteSandbox2, listSandboxes: listSandboxes2, enforce: enforce2 } = await Promise.resolve().then(() => (init_src3(), src_exports2));
...
app.post("/api/sandbox/:profileId/enforce", async (req, res) => {
  try {
    const { enforce: enforce2 } = await Promise.resolve().then(() => (init_src3(), src_exports2));
    const { profileId } = req.params;
    const { tool, args } = req.body;
    res.json(enforce2(tool, profileId, args || {}));
  } catch (e) {
...
app.get("/api/sandbox-escapes", async (req, res) => {
  try {
    ...
    const content = import_node_fs.default.readFileSync(ESCAPE_PATH, "utf-8");
