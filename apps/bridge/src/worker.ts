...
<body>
  ...
  ${results.length > 0 ? `<table>
    ...
    ${(results as any[]).map((e: any) => `<tr>
      ...
      // ─── Scoreboard Read Endpoints (BRIDGE_DB) ─────────────────────────

      // GET /api/events/weekly - completed task count for scoreboard Outputs metric
      if (path === '/api/events/weekly') {
        ...
        if (weekParam === 'current') {
          ...
          // Accept ISO date string as week start
          const d = new Date(weekParam);
          if (isNaN(d.getTime())) return json({ error: 'Invalid week param. Use "current" or ISO date.' }, 400);
          start = d.toISOString();
          end = new Date(d.getTime() + 7 * 86400000).toISOString();
        }

        const kindFilter = url.searchParams.get('kind'); // optional: TASK_CREATED, etc.

        let query = "SELECT COUNT(*) as total FROM events WHERE created_at >= ? AND created_at < ?";
        const params: any[] = [start, end];
        if (kindFilter) { query += " AND kind = ?"; params.push(kindFilter); }

        const row = await env.BRIDGE_DB.prepare(query).bind(...params).first<{ total: number }>();

        // Also grab a breakdown by kind
        let breakdownQuery = "SELECT kind, COUNT(*) as count FROM events WHERE created_at >= ? AND created_at < ? GROUP BY kind ORDER BY count DESC";
        const { results: breakdown } = await env.BRIDGE_DB.prepare(breakdownQuery).bind(start, end).all();
      ...
      // GET /api/metrics/rollbacks - rollback count for scoreboard Quality metric
      if (path === '/api/metrics/rollbacks') {
        if (!env.BRIDGE_DB) return json({ error: 'BRIDGE_DB not bound' }, 500);
        ...
        if (weekParam === 'current') {
          ...
          const d = new Date(weekParam);
          if (isNaN(d.getTime())) return json({ error: 'Invalid week param' }, 400);
          start = d.toISOString();
          end = new Date(d.getTime() + 7 * 86400000).toISOString();
        }

        // Count rollback events from BRIDGE_DB
        const rollbackRow = await env.BRIDGE_DB.prepare(
          "SELECT COUNT(*) as rollbacks FROM events WHERE kind = 'ROLLBACK' AND created_at >= ? AND created_at < ?"
        ).bind(start, end).first<{ rollbacks: number }>();

        // Count total runs for the ratio
        const totalRow = await env.BRIDGE_DB.prepare(
          "SELECT COUNT(*) as total FROM events WHERE created_at >= ? AND created_at < ?"
        ).bind(start, end).first<{ total: number }>();

        const rollbacks = rollbackRow?.rollbacks ?? 0;
        ...
        return json({
          ...
          rollbacks,
          total_events: total,
          rollback_rate: Math.round(rate * 10000) / 10000, // 4 decimal places
        });
      }
