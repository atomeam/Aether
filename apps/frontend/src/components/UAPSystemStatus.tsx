...
export function UAPSystemStatus() {
  ...
  const fetchStatus = async () => {
    try {
      // Try multiple endpoints to get system data
      const [stackRes, agentsRes] = await Promise.all([
        fetch('http://localhost:3000/api/stack'),
        fetch('http://localhost:3000/api/agents')
      ]);

      if (stackRes.ok && agentsRes.ok) {
        const stackData = await stackRes.json();
        const agentsData = await agentsRes.json();
        
        // Map backend data to UAP system status
        setSystemStatus({
          sensors: 5, // Static for now - would need dedicated endpoint
          anomalies: agentsData.curator === 'active' ? 12 : 8,
          alerts: agentsData.circuitBreaker === 'closed' ? 3 : 0,
          clusters: 2,
          reports: 45,
          users: 23,
          devices: 156,
      ...
      // Use simulated data if local API not available
      setSystemStatus({
        sensors: 5,
        anomalies: 12,
        alerts: 3,
      ...
      setError('Using simulated data - local backend not available');
    } finally {
      setLoading(false);
    }
  };
